    ...
    return (
      <div className={`${props.className}`} {...props}>
        ...
        <div className='w-28 h-28 p-3 bg-[#2d454750] backdrop-blur-xl rounded-lg uppercase font-bold flex items-center relative'>
          <p className={`absolute bottom-3 right-3 text-[#DFFAFF50] [text-shadow:_0px_0px_10px_#81c7d270] ${props.skillName.length <= 3 ? 'text-5xl' : props.skillName.length <= 4 ? 'text-3xl' : props.skillName.length <= 5 ? 'text-2xl': 'text-xl'}`}>{props.skillName}</p>
        </div>
      </div>
    )
  }
