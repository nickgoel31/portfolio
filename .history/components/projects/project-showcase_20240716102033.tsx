                        ...
                        {!repoData ? (
                            ...
                            <>
                                { repoData?.length === 0 ? "No Files": (
                                    <>
                                        {repoData.slice(0,20).map((cont:any,index:number) => (
            ...
            {showcaseType === "Video" && (
                <div className='w-full h-full'>
                    {currentProject.projectVideoPath !== "" ? (
                        <iframe src={`https://www.youtube.com/embed/${currentProject.projectVideoPath}`} title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" className='w-full h-full'></iframe>
                    ):(
                        <div className='w-full h-full flex items-center justify-center'>
                            <p className='text-xs font-medium opacity-70'>
                                No Video Showcase
...
export const MobileProjectShowcase = () => {
    ...
    },[currentProject, showcaseType])
  return (
    <div className='w-full h-full flex flex-col gap-8 items-center justify-center py-8 px-6'>
        <div className={`flex items-center justify-center text-xs font-medium p-1 px-2 rounded-full border ${currentProject.status === "COMPLETED" ? 'border-green-400 text-green-300': 'border-yellow-500 text-yellow-300'}`}>
            {currentProject.status === "COMPLETED" ? "Ready" : "In Progress"}
        </div>
        <div className='flex gap-3 items-center '>
            <button onClick={() => {
                setShowcaseType("Github")
                setLoading(true)
            }} className={`rounded-full p-3 px-6 border border-[#274552] bg-[#0e171b] flex items-center justify-center gap-2 transition-custom hover:border-[#89bcd2] hover:bg-[#89bcd2] hover:shadow-contactLinks hover:text-[#0e171b] font-semibold text-[#a4c4d1] ${showcaseType === "Github" && 'border-[#89bcd2] bg-[#89bcd2] shadow-contactLinks !text-[#0e171b] '}`}>
                <BsGithub />
                Github
            </button>
            <button onClick={() => {
                setShowcaseType("Video")
                setLoading(true)
            }} className={`rounded-full p-3 px-6 border border-[#274552] bg-[#0e171b] flex items-center justify-center gap-2 transition-custom hover:border-[#89bcd2] hover:bg-[#89bcd2] hover:shadow-contactLinks hover:text-[#0e171b] font-semibold text-[#a4c4d1] ${showcaseType === "Video" && 'border-[#89bcd2] bg-[#89bcd2] shadow-contactLinks !text-[#0e171b]'}`}>
                <Video />
                Showcase
            </button>
            <button onClick={() => {
                setShowcaseType("Link")
                setLoading(true)
            }} className={`rounded-full p-3 px-6 border border-[#274552] bg-[#0e171b] flex items-center justify-center gap-2 transition-custom hover:border-[#89bcd2] hover:bg-[#89bcd2] hover:shadow-contactLinks hover:text-[#0e171b] font-semibold text-[#a4c4d1] ${showcaseType === "Link" && 'border-[#89bcd2] bg-[#89bcd2] shadow-contactLinks !text-[#0e171b]'}`}>
                <LinkIcon />
                Link
            </button>
        </div>

        <div className='w-[100%] aspect-video overflow-hidden z-[10] rounded-md relative  border border-[#274552] bg-[#0b1114]'>
            {showcaseType === "Github" && (
                <>
                {currentProject.projectGithub ? (
                    <div className='w-full h-full flex flex-col relative '>
            ...
            {showcaseType === "Link" && (
                <div className='w-full h-full relative'>
                    {currentProject.projectLink ? (
                        ...
                        <div className='w-full h-full flex items-center justify-center absolute left-0 top-0 z-[10]'></div>
                        <iframe src={currentProject.projectLink} name="myiFrame" loading='lazy' seamless scrolling="no" className='w-full h-full github-repo' ></iframe>
                        </>
                    ):(
                        <div className='w-full h-full flex items-center justify-center'>
                            <p className='text-xs font-medium opacity-70'>
