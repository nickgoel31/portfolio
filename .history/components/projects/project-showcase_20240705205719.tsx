...
const ProjectShowcase = () => {
    ...
    const {repoCommits, repoData} = useRepoData()
  return (
            ...
            {showcaseType === "Github" && (
                <div className='w-full h-full flex flex-col relative '>
                    ...
                    <div className='absolute bottom-6 left-0 w-full flex items-center justify-center z-[100]'>
                        <Link href={"/gdfgdfg"} className='rounded-full p-3 px-6 border border-[#274552] bg-[#0e171b] text-xs font-medium flex items-center gap-2'>
                            <BsGithub size={15}/>
                            View on github
                        </Link>
                    </div>

                    <div className='flex flex-col w-full github-repo'>
                        {!repoData ? (
                            <></>
                        ):(
                            <>
                                {repoData?.length === 0 ? "No Files": (
                                    <>
                                        {repoData.slice(0,10).sort.map((cont,index) => (
                                            <div key={index} className='w-full h-8 p-2 px-4 text-xs font-medium flex items-center justify-between border-b border-[#274552] hover:bg-white/5'>
                                                <div className='flex items-center gap-2'>
                                                    ...
                                                    <Link href='gdfg' target='_blank' className='opacity-70 hover:text-blue-500 hover:underline'>{cont.name}</Link>
                                                </div>
                        ...
                        <div className='w-full h-8 p-2 px-4 text-xs font-medium flex items-center justify-between border-b border-[#274552] hover:bg-white/5'>
                            <div className='flex items-center gap-2'>
                                <File size={15}/>
                                <Link href='gdfg' target='_blank' className='opacity-70 hover:text-blue-500 hover:underline'>.gitignore</Link>
                            </div>
                            <p className='opacity-70'>file</p>
                        </div>
