  ...
  return (
    <div className='w-full h-full gap-1 relative z-[100]'>
        ...
        {cards.map((card) => (
             ...
             className={`w-full p-4 bg-[#0A1112] absolute rounded-lg shadow-projectCards hover:translate-y-[-10%] transition-all duration-500 cursor-pointer border-2 border-[#c4d3d559]  ${currentProjectIndex === card.id && '!border-[#528992]'} ${card.id === cards.length - 1 && '!'}`}
             style={{
               top: `calc(${card.top}%)`,
               zIndex: card.zIndex,
