...
export const SkillBox = ({...props}:Props) => {
  const [modal, setModal] = useState(false)
    return (
      <>
        {modal && (
          <div className="fixed bottom-0 w-full p-6 py-8 z-[10] bg-[#0f1618] border border-[#1e2e32] left-0">
            <div className="absolute right-4 top-4" onClick={() => setModal(false)}>
              <X />
            </div>
            <div className="space-y-2 flex flex-col item">
              <h1 className="font-bold text-4xl">
                {props.skillName}
              </h1>
              <p className="px-1 py-0.5 rounded-full bg-slate-500">{props.skillLevel || "beginner"}</p>
            </div>
          </div>
        )}
